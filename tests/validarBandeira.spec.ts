...
describe('validateCardBrand', () => {
  ...
  it('deve identificar Aura válido', () => {
    expect(validateCardBrand('5078601870000127980')).toEqual({ brand: 'Aura', valid: true });
  });
  it('deve identificar Aura inválido', () => {
    expect(validateCardBrand('5078601870000127981')).toEqual({ brand: 'Aura', valid: false });
  });

  // Voyager
  it('deve identificar Voyager válido', () => {
    expect(validateCardBrand('869923456789012')).toEqual({ brand: 'Voyager', valid: true });
  });
  it('deve identificar Voyager inválido', () => {
    expect(validateCardBrand('869923456789013')).toEqual({ brand: 'Voyager', valid: false });
  });
